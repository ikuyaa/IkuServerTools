...
public class WarpDenyCommand {
    private static final String WARP_ARG = "warp";
    private static final String PLAYER_ARG = "player";

    private WarpDenyCommand() {}
    ...
    private static int execute(CommandSourceStack source, String warpName, UUID targetUuid, String targetName) {
        ...
        if (warp.creator().equals(targetUuid)) {
            CommandHelpers.failure(source, "You cannot remove the warp owner's access.");
            return 0;
        }

        boolean removed = WarpData.get().removeAllowedPlayer(warpName, targetUuid);
        if (!removed) {
            CommandHelpers.failure(source, "That player does not currently have access to this warp.");
            return 0;
        }
        ...
        CommandHelpers.success(source, () -> Component.literal("Removed access for ")
                .withStyle(ChatFormatting.GREEN)
                .append(Component.literal(targetName).withStyle(ChatFormatting.GOLD, ChatFormatting.UNDERLINE))
                .append(Component.literal(" from warp ").withStyle(ChatFormatting.GREEN))
                .append(Component.literal(warpName).withStyle(ChatFormatting.GOLD, ChatFormatting.UNDERLINE))
